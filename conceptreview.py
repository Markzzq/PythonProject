...
if __name__ == '__main__':
    ...
    df_concept_list = pd.read_csv('concept_em_list.csv')
    df_concept = df_concept_list[['板块名称', '板块代码']]
    ...
    for row_index, row in df_concept.iterrows():
        try:
            ...
            # 日线数据
            """
            用东方财富的概念列表 搜索出的历史数据 日线
            adjust=""; choice of {'': 不复权, 默认; "qfq": 前复权, "hfq": 后复权}

            """

            # 东方财富的概念板块接口
            stock_board_concept_hist_em_df = ak.stock_board_concept_hist_em(symbol=row_name, period="daily",
            ...
            N = stock_board_concept_hist_em_df.shape[0]

            # 计算初级数据  均线策略因子
            CLOSE = stock_board_concept_hist_em_df['收盘']
            ...
            if varAll:
                anyData = {'概念': row_name,'OPEN': stock_board_concept_hist_em_df['开盘价'][N-1], 'CLOSE': stock_board_concept_hist_em_df['收盘价'][N-1]}
    ...
    print(dfResult)
    #### 结果集输出到csv文件 ####
    file_name = f"{END_DATE}_concept_em.csv"
